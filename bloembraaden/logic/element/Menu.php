...
class Menu extends BaseElement
{
    ...
    public function putItem(int $item_id, int $to_item_id, string $relative): bool
    {
        ...
        if (null === $this->getDB()->fetchElementRow(new Type('menu_item'), $item_id)) {
            $items_belong_to_this_instance = false;
        }
        if ($to_item_id > 0 and
            null === $this->getDB()->fetchElementRow(new Type('menu_item'), $to_item_id)) {
            $items_belong_to_this_instance = false;
        }
        ...
        if ($relative === 'child') {
            return $this->getDB()->underMenuItem($this->getId(), $item_id, $to_item_id);
        } else { // order $item_id right before $to_item_id
            return $this->getDB()->orderMenuItem($this->getId(), $item_id, $to_item_id);
        }
    }
    ...
    public function completeRowForOutput(): void
    {
        ...
        $this->row->__menu__ = array(
            '__item__' => ($menu_items = $this->getDB()->fetchMenuItems($this->getId())),
        );
