...
//            die();
    ...
    /**
     * @param int $variant_id
     * @param int|null $quantity the amount to add or subtract, when null the quantity_in_stock will be set to null
     * @return bool success
     */
    public function updateVariantQuantityInStock(int $variant_id, ?int $quantity): bool
    {
        ...
        if (1 === $statement->rowCount()) {
            ...
            $statement = null;
            // manage in stock switch when going in and out of stock
            if (null !== $quantity) {
                $this->updateRowAndReturnSuccess('cms_variant', array('in_stock' => (0 < $quantity)), $variant_id);
            }
...
//            $statement->execute(array($ip_address, $token));
    ...
    {
        $never = array('_cache', '_stale', '_ci_ai', '_admin', '_admin_login_attempt', '_client', '_locker', '_payment', '_system', '_session', '_sessionvars', '_instance_domain', '_order_number', '_image_slug_history');
    ...
    {
        @include "$this->cache_folder$key.serialized"; // @ sets error level to 0, which will be caught by error handler
        return $val ?? false;
    }
}
